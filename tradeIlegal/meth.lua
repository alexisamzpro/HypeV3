...
local Price = math.random(70,80) * 100

local Position = {
    -- VOS POINTS ICI
    Recolet={x=-102.505,y=1908.63,z=196.105, distance=10},
    traitement={x=95.021,y=3749.7,z=39.7205, distance=3},
    traitement2={x=714.332,y=4198.16,z=39.8813, distance=3},
    traitement3={x=1391.95,y=3605.54,z=37.9419, distance=3},
    vente={x=295.256,y=-1004.98,z=28.335, distance=5}
}
...
-------------------------Bloc Pour rajouter un traitement-------------------------------------------
        ...
        if not IsInVehicle() then
          if distanceWeedFarm < Position.traitement.distance then
             ShowInfo('~b~Appuyer sur ~g~E~s~ pour ~o~traiter ~s~de l\'~b~Ephedrine', 0)
             if IsControlJustPressed(1, 38) then
                   weedcount = 0
                  ...
                  if weedcount ~= 0 then
                          ...
                          TriggerEvent("vmenu:anim" ,"pickup_object", "pickup_low")
                          Wait(2500)
                          ShowMsgtime.msg = '~g~ + 1 ~b~Matière illégale (33% traitées)'
                          ShowMsgtime.time = 150
        ...
        if not IsInVehicle() then
          if distanceWeedFarm < Position.traitement2.distance then
             ShowInfo('~b~Appuyer sur ~g~E~s~ pour ~o~traiter ~s~de la ~b~Matière illégale (33% traitées)', 0)
             if IsControlJustPressed(1, 38) then
                   weedcount = 0
                   -- TriggerEvent("player:getQuantity", 6, function(data)
                   --      weedcount = data.count
                  ...
                  Wait(100)
                  if weedcount ~= 0 then
                          ShowMsgtime.msg = '~g~ Traitement ~b~Matière illégale (33% traitées)'
                          ShowMsgtime.time = 250
                          TriggerEvent("vmenu:anim" ,"pickup_object", "pickup_low")
                          Wait(2500)
                          ShowMsgtime.msg = '~g~ + 1 ~b~Matière illégale (66% traitées)'
                          ShowMsgtime.time = 150

                          TriggerEvent("player:looseItem", 10, 1) --10
                          TriggerEvent("player:receiveItem", 11, 1) --11
                  else
                          ShowMsgtime.msg = "~r~ Vous n'avez pas de Matière illégale (33% traitées) !"
                          ShowMsgtime.time = 150
                  end
             end
           end
        ...
        if not IsInVehicle() then
          if distanceWeedFarm < Position.traitement3.distance then
             ShowInfo('~b~Appuyer sur ~g~E~s~ pour ~o~traiter ~s~de la ~b~Matière illégale (66% traitées)', 0)
             if IsControlJustPressed(1, 38) then
                   weedcount = 0
                   -- TriggerEvent("player:getQuantity", 6, function(data)
                   --      weedcount = data.count
                  ...
                  Wait(100)
                  if weedcount ~= 0 then
                          ShowMsgtime.msg = '~g~ Traitement ~b~Matière illégale traitée (66% traitées)'
                          ShowMsgtime.time = 250
                          TriggerEvent("vmenu:anim" ,"pickup_object", "pickup_low")
                          Wait(2500)
                          ...
                          ShowMsgtime.msg = "~r~ Vous n'avez pas de Matière illégale (66% traitées) !"
                          ShowMsgtime.time = 150
                  end
             end
           end
...
-------------------------Fin Du Bloc Pour rajouter un traitement-------------------------------------------
        ...
        if not IsInVehicle() then
          if distanceWeedFarm < Position.vente.distance then
             ShowInfo('~b~Appuyer sur ~g~E~s~ pour ~o~vendre ~s~de la ~b~Meth', 0)
             if IsControlJustPressed(1, 38) then
                   weedcount = 0
                  ...
                  if weedcount ~= 0 then
                          ...
                          TriggerEvent("vmenu:anim" ,"pickup_object", "pickup_low")
                          Wait(2500)
                          ShowMsgtime.msg = '1 Meth vendue~n~~g~( +'..Price..'$ )'
                          ShowMsgtime.time = 150
