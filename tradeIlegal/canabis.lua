...
local Price = math.random(30,35) * 100

local Position = {
    -- VOS POINTS ICI
    Recolet={x=2226.02,y=5577.18,z=52.8396, distance=10, bli},
    traitement={x=3689.93,y=4562.71,z=24.1831, distance=5},
    vente={x=36.8642,y=-1026.84,z=29.5057, distance=5}
}
    ...
    while true do
        ...
        if not IsInVehicle() then
          if distanceWeedFarm < Position.Recolet.distance then
             ShowInfo('~b~Appuyer sur ~g~E~s~ pour ~o~récolter ~s~du ~b~Chanvre', 0)
             if IsControlJustPressed(1,38) then
                  ...
                  if weedcount < 30 then
                          ShowMsgtime.msg = '~g~ Récolte de ~b~Chanvre'
                          ShowMsgtime.time = 250
                          TriggerEvent("vmenu:anim" ,"pickup_object", "pickup_low")
                          Wait(2500)
                          ShowMsgtime.msg = '~g~ + 1 ~b~Chanvre'
        ...
        if not IsInVehicle() then
          if distanceWeedFarm < Position.traitement.distance then
             ShowInfo('~b~Appuyer sur ~g~E~s~ pour ~o~rouler ~s~du ~b~Cannabis', 0)
             if IsControlJustPressed(1,38) then
                  weedcount = 0
                  -- TriggerEvent("player:getQuantity", 4, function(data)
                  --      weedcount = data.count
        ...
        if not IsInVehicle() then
          if distanceWeedFarm < Position.vente.distance then
             ShowInfo('~b~Appuyer sur ~g~E~s~ pour ~o~vendre ~s~du ~b~Cannabis', 0)
             if IsControlJustPressed(1,38) then
                  weedcount = 0
                  ...
                  if weedcount ~= 0 then
                          ...
                          TriggerEvent("vmenu:anim" ,"pickup_object", "pickup_low")
                          Wait(2500)
                          ShowMsgtime.msg = '1 Cannabis roulé vendu~n~~g~( +'..Price..'$ )'
                          ShowMsgtime.time = 150
